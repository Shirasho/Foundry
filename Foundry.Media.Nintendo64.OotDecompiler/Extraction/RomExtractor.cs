...
{
    internal sealed class RomExtractor : IRomExtractor
    {
        private const int FileTableOffset = 0x12F70;
        private readonly IRomData Data;

        public RomExtractor(IRomData romData)
        {
            Data = romData;
        }
        ...
        {
            var output = options.OutputDirectory.CombineDirectory("baserom");
            if (!output.Exists)
            {
                output.Create();
            }

            var bigEndianRomData = await Data.ConvertToAsync(ERomFormat.BigEndian, cancellationToken);
            var data = await bigEndianRomData.GetDataAsync();

            await FileNames.ForEachParallelAsync(Environment.ProcessorCount, async (fileName, token) =>
            {
                var fileInfo = new FileInfo(Path.Combine(output.FullName, fileName));
                if (fileInfo.Exists)
                {
                    fileInfo.Delete();
                }

                // Unfortunately we do not have the ability to pass in an index in ForEachParallel right now,
                // so we need to do a lookup.

                await ExtractFileAsync(data, fileInfo, Array.IndexOf(FileNames, fileName), token);
        ...
        private async ValueTask ExtractFileAsync(ReadOnlyMemory<byte> data, FileInfo output, int fileIndex, CancellationToken cancellationToken)
        {
            int entryOffset = FileTableOffset + 16 * (int)fileIndex;
            var metadataWindow = data.Slice(entryOffset, 16);
            int virtStart = ReadInt(metadataWindow, 0);
            int virtEnd = ReadInt(metadataWindow, 4);
            int physStart = ReadInt(metadataWindow, 8);
            int physEnd = ReadInt(metadataWindow, 12);
            ...
            using var fs = output.Open(FileMode.Create, FileAccess.Write, FileShare.None);
            data = data.Slice(physStart, size);
            if (compressed)
            {
            ...
            static int ReadInt(ReadOnlyMemory<byte> buffer, int offset)
            {
                Span<byte> localBuffer = stackalloc byte[4];
                buffer.Slice(offset, 4).Span.CopyTo(localBuffer);

                // We are working with N64 files as big-endian.
                if (BitConverter.IsLittleEndian)
                {
                    localBuffer.Reverse();
                }

                return BitConverter.ToInt32(localBuffer);
            }
