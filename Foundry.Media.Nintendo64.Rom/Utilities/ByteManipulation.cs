...
namespace Foundry.Media.Nintendo64.Rom.Utilities
{
    ...
    public static class ByteManipulation
    {
        ...
        /// <summary>
        /// Swaps the endian of a number if the current computer architecture
        /// stores numbers in little endian.
        /// </summary>
        /// <param name="number">The number in the endianness of the current architecture.</param>
        /// <remarks>
        /// While the ROM processing logic itself works on big endian bytes, certain numeric reads
        /// (table sizes for example) must be byte swapped.
        /// </remarks>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static uint SwapEndianIfNecessary(uint number)
        {
            return BitConverter.IsLittleEndian ? BinaryPrimitives.ReverseEndianness(number) : number;
        }
